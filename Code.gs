...
function doGet(e) {
  ...
  if (action === 'ping') return jsonResponse({ status: 'ok', message: 'Техноточка API работает!' });
  return jsonResponse({ status: 'ok', hint: 'Используй POST-запросы для работы с API' });
}
